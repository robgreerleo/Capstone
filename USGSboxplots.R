# USGS Boxplot Functions - https://waterdata.usgs.gov/blog/boxplots/
  ...
  if(logY){
    ...
    return(c(basic_elements,
             scale_y_continuous(sec.axis = dup_axis(label = NULL,
                                                    name = NULL),
                                expand = expansion(mult = c(0, 0)),
                                breaks = pretty(c(lower_limit,upper_limit), n = 10),
...
ggplot_box_legend <- function(family = "serif"){
  ...
  explain_plot <- ggplot() +
    ...
    geom_text(aes(x = 1, y = 950, label = "20000"), hjust = 0.5) +
